...
def extract_subsections(file_path):
    """Extracts subsection names from comments labeled with '# Chapt {name}'."""
    subsections = []

    try:
        with open(file_path, "r", encoding="utf-8") as file:
            for lineno, line in enumerate(file, start=1):
                # Match subsections marked with '# Chapt {name}'
                match = re.match(r"#\s*Chapt\s+(.+)", line.strip())
                if match:
                    subsections.append({
                        "type": "subsection",
                        "name": match.group(1),
...
if __name__ == "__main__":
    ...
    if grouped_entries:
        ...
        if not args.json and not args.csv:
            print("No output format specified. Use --json, --csv, or both.")
    else:
